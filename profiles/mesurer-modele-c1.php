...
$zones=[];
foreach($m['rows'] as $r){
 $a=$r['attackerType'];$d=$r['defenderType'];$x=$wins[$a][$d];if($r['side']==='defender')$x=1-$x;
 $mirror=$a===$d;$rx=($x==.5||$x<=.05||$x>=.95)?.05:.1;
 $y=.5;$ry=1.0;
 if($mirror&&$a==='soldier'){$y=1.0;$ry=.05;}
 elseif($mirror){$y=.1;$ry=$a==='archer'?.1:.05;}
...
foreach($data['zonesDocument']['zones'] as $i=>&$zone){$zone['source']['originalCenter']=['x'=>$m['rows'][$i]['winRate'],'y'=>$m['rows'][$i]['rawCasualtyRatio']];$zone['source']['modifiedManually']=true;}unset($zone);
$evaluator=new Waar\MicroCombat\Experiment\AcceptanceZoneEvaluator();$summary=[];
foreach($m['rows'] as $i=>$row)$summary[]=['id'=>$row['id'],'winRate'=>$row['winRate'],'lossRatio'=>$row['rawCasualtyRatio'],'accepted'=>$evaluator->evaluate($row['winRate'],$row['rawCasualtyRatio'],$zones[$i])==='inside'];
