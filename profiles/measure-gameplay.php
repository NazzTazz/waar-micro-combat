...
foreach($m['rows'] as $row){
    if(($expected[$row['scenarioId']]??null)===$row['side'] && $row['winRate']!==1)throw new RuntimeException('Expected 100/100 observed wins: '.$row['id']);
    if($row['id']==='spearman-vs-knight/attacker' && $row['rawLossRatio']<0.9)throw new RuntimeException('Attacking spearmen were not decimated.');
}
