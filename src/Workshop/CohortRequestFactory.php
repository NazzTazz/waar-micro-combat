...
final readonly class CohortRequestFactory
{
    ...
    public static function assertProvenance(array $actual, array $requested): void
    {
        foreach (['policyVersion' => self::POLICY_VERSION, 'samplingProtocol' => self::SAMPLING_PROTOCOL,
            ...
            if (($actual[$key] ?? null) !== $value) {
                throw new \RuntimeException('Politique de conséquences du runtime incompatible : reconstruisez Rust et remesurez.');
            }
        }
    }
    ...
    public function combat(
        ...
        array $defender,
        int $seed,
        string $attackerWeather,
        string $defenderWeather,
        array $attackerModifiers = [],
        array $defenderModifiers = [],
        string $traceLevel = 'full',
        string $attackerIdentity = 'A',
        string $defenderIdentity = 'B'
    ): array {
        return [
