...
final readonly class BattleConsequences
{
    ...
    public function apply(BattleResult $result, EngineProfile $profile): array
    {
        ...
        foreach ([CombatSide::Attacker, CombatSide::Defender] as $side) {
            ...
            $lost = $captured = $free = $economicLoss = 0;
            $units = [];
            foreach (UnitType::cases() as $type) {
                $raw = $outcome->unit($type);
                $applied = self::floorPercent($raw->dead, $profile->lossCompressionPercent);
                $before = $raw->initial - $applied;
                ...
                $lost = FixedPoint::checkedAdd($lost, $applied);
                $captured = FixedPoint::checkedAdd($captured, $prisoners);
                $free = FixedPoint::checkedAdd($free, $remaining);
                if ($raw->initial !== $applied + $prisoners + $remaining) {
                    throw new \LogicException('Conservation des effectifs rompue.');
                }
    ...
    {
        if ($value < 0 || $percent < 0 || $percent > 100) {
            throw new \InvalidArgumentException('Pourcentage ou effectif invalide.');
        }
