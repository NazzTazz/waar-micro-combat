...
final readonly class EngineProfileMigrator
{
    ...
    public function migrate(array $source): array
    {
        ...
        if($schema===EngineProfile::SCHEMA_VERSION){
            ...
            return ['profile'=>$profile,'migration'=>['performed'=>false,'sourceSchema'=>$schema,'targetSchema'=>$schema,'modelVersion'=>EngineProfile::MODEL_VERSION,'measurementsObsolete'=>false,'newFields'=>[],'obsoleteFields'=>[],'notes'=>[]]];
        }
        ...
        return ['profile'=>$validated,'migration'=>[
            ...
            'newFields'=>['units.*.baseAccuracy=0.15','units.*.accuracySpread=0','units.*.strikesPerAttack=1','weather.*.*.baseAccuracy=1','combat.surrenderEnabled=false','combat.surrenderDeadPercent=20','combat.tieBreakCriterion=economic'],
