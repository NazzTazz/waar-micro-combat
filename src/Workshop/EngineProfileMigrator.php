...
final readonly class EngineProfileMigrator
{
    ...
    public function migrate(array $source): array
    {
        ...
        foreach (['id', 'label', 'relations'] as $field) {
            if (array_key_exists($field, $source)) {
                $profile[$field] = $source[$field];
            }
        }
        foreach (EngineProfile::UNIT_COSTS as $type => $unused) {
            $legacy = $source['units'][$type] ?? null;
            if ($legacy === null) {
                $profile['units'][$type] = null;
                continue;
            }
            if (!is_array($legacy)) {
                throw new \InvalidArgumentException('Ancienne fiche d’unité invalide : '.$type);
            }
            foreach (['attack', 'structure', 'defendingEfficiency', 'cost', 'capturable'] as $field) {
                if (array_key_exists($field, $legacy)) {
                    $profile['units'][$type][$field] = $legacy[$field];
                }
            }
            $profile['units'][$type]['baseAccuracy'] = '0.15';
            $profile['units'][$type]['accuracySpread'] = '0';
            $profile['units'][$type]['strikesPerAttack'] = 1;
        }
        foreach (EngineProfile::WEATHER as $condition) {
            foreach (EngineProfile::UNIT_COSTS as $type => $unused) {
                $legacy = $source['weather'][$condition][$type] ?? null;
                if (is_string($legacy) || is_int($legacy)) {
                    $profile['weather'][$condition][$type]['attack'] = (string)$legacy;
                }
                $profile['weather'][$condition][$type]['baseAccuracy'] = '1';
            }
        }
        $combat = is_array($source['combat'] ?? null) ? $source['combat'] : [];
        foreach (['maxRounds', 'lossCompressionPercent', 'capturePercent'] as $field) {
            if (array_key_exists($field, $combat)) {
                $profile['combat'][$field] = $combat[$field];
            }
        }
        if (in_array($combat['tieBreakPolicy'] ?? null, ['defender', 'draw'], true)) {
            $profile['combat']['equalityPolicy'] = $combat['tieBreakPolicy'];
        }
