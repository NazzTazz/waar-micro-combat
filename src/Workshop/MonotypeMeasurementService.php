...
final class MonotypeMeasurementService
{
    ...
    public function measure(array $profileValues, string $weather = 'neutral', int $baseSeed = 42, int $iterations = 100): array
    {
        if ($iterations < 1 || $iterations > 100) {
            throw new \InvalidArgumentException('La mesure accepte de 1 à 100 répétitions.');
        }
        if ($baseSeed < 0 || $baseSeed > 2147483647) {
            throw new \InvalidArgumentException('Seed invalide.');
        }
        ...
        foreach ($batch['scenarios'] as $scenario) {
            foreach (['attacker', 'defender'] as $side) {
                if (!isset($scenario['result'][$side.'RawWoundedByType'])) {
                    throw new \RuntimeException('Runtime obsolète : reconstruisez Rust pour mesurer les blessés bruts.');
                }
            }
        }
        foreach ($batch['scenarios'] as $scenario) {
            ...
            [$attacking,$defending] = explode('-vs-', $id, 2);
            foreach (['attacker', 'defender'] as $side) {
                $prefix = $side === 'attacker' ? 'attacker' : 'defender';
                $initialByType = $result[$prefix.'InitialByType'];
                $rawByType = $result[$prefix.'RawDeathsByType'];
                $projected = $result[$prefix.'ProjectedByType'];
                $type = $side === 'attacker' ? $attacking : $defending;
                $index = array_search($type, $batch['unitOrder'], true);
                if ($index === false) {
                    throw new \RuntimeException('Type monotype absent du batch.');
                }
                $initial = $initialByType[$index] * $iterations;
                $raw = $rawByType[$index];
                $p = $projected[$index];
                $wins = $result[$prefix.'Wins'];
