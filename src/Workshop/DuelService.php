...
final class DuelService
{
    ...
    {
        $errors=[];$armies=[];
        foreach(['A','B']as$name){$army=$request['armies'][$name]??null;if(!is_array($army)||([]!==$army&&array_is_list($army))){$errors[]=['code'=>'invalid_army','path'=>'armies.'.$name,'message'=>'Armée invalide.'];continue;}foreach(array_diff(array_keys($army),array_keys(EngineProfile::UNIT_COSTS))as$type)$errors[]=['code'=>'unknown_unit','path'=>'armies.'.$name.'.'.$type,'message'=>'Type d’unité inconnu.'];$total=0;$normalized=[];foreach(EngineProfile::UNIT_COSTS as$type=>$cost){$value=$army[$type]??0;if(!is_int($value)||$value<0||$value>1000000)$errors[]=['code'=>'out_of_range','path'=>'armies.'.$name.'.'.$type,'message'=>'Effectif entier attendu entre 0 et 1 000 000 par type.'];else{$normalized[$type]=$value;$total=FixedPoint::checkedAdd($total,$value);}}if($total===0)$errors[]=['code'=>'empty_army','path'=>'armies.'.$name,'message'=>'Chaque camp doit contenir au moins une unité.'];$armies[$name]=$normalized;}
