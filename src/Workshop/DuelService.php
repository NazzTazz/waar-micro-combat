...
final class DuelService
{
    ...
    public function __construct(?CohortRuntime $runtime = null, private readonly CohortRequestFactory $requests = new CohortRequestFactory())
    {
    ...
    public function simulate(array $request, bool $summary = false): array
    {
        ...
        $armies = [];
        foreach (['A', 'B'] as $name) {
            $army = $request['armies'][$name] ?? null;
            if (!is_array($army) || ([] !== $army && array_is_list($army))) {
                $errors[] = ['code' => 'invalid_army', 'path' => 'armies.'.$name, 'message' => 'Armée invalide.'];
                continue;
            }
            foreach (array_diff(array_keys($army), array_keys(EngineProfile::UNIT_COSTS)) as $type) {
                $errors[] = ['code' => 'unknown_unit', 'path' => 'armies.'.$name.'.'.$type, 'message' => 'Type d’unité inconnu.'];
            }
            $total = 0;
            $normalized = [];
            foreach (EngineProfile::UNIT_COSTS as $type => $cost) {
                ...
                if (!is_int($value) || $value < 0 || $value > 1000000) {
                    $errors[] = ['code' => 'out_of_range', 'path' => 'armies.'.$name.'.'.$type, 'message' => 'Effectif entier attendu entre 0 et 1 000 000 par type.'];
                } else {
                    $normalized[$type] = $value;
                    $total = FixedPoint::checkedAdd($total, $value);
                }
            }
            if ($total === 0) {
                $errors[] = ['code' => 'empty_army', 'path' => 'armies.'.$name, 'message' => 'Chaque camp doit contenir au moins une unité.'];
            }
            $armies[$name] = $normalized;
        }
        $seed = $request['seed'] ?? null;
        if (!is_int($seed) || $seed < 0 || $seed > 2147483647) {
            $errors[] = ['code' => 'invalid_seed', 'path' => 'seed', 'message' => 'Seed entière attendue entre 0 et 2³¹−1.'];
        }
        ...
        if (!is_array($weather) || array_is_list($weather)) {
            $errors[] = ['code' => 'invalid_weather', 'path' => 'weather', 'message' => 'Une météo est requise pour chaque camp.'];
            $weather = ['A' => 'neutral', 'B' => 'neutral'];
        }
        foreach (['A', 'B'] as $camp) {
            if (!is_string($weather[$camp] ?? null) || !in_array($weather[$camp], EngineProfile::WEATHER, true)) {
                $errors[] = ['code' => 'unknown_weather', 'path' => 'weather.'.$camp, 'message' => 'Condition météo inconnue.'];
            }
        }
        $modifiers = $request['modifiers'] ?? ['A' => [], 'B' => []];
        if (!is_array($modifiers) || array_is_list($modifiers)) {
            $errors[] = ['code' => 'invalid_modifiers', 'path' => 'modifiers', 'message' => 'Les effets doivent être séparés par camp.'];
            $modifiers = ['A' => [], 'B' => []];
        }
        foreach (['A', 'B'] as $camp) {
            if (!is_array($modifiers[$camp] ?? []) || !array_is_list($modifiers[$camp] ?? [])) {
                $errors[] = ['code' => 'invalid_modifiers', 'path' => 'modifiers.'.$camp, 'message' => 'Liste d’effets invalide.'];
            }
        }
        $active = [];
        foreach ($armies as $army) {
            foreach ($army as $type => $count) {
                if ($count > 0) {
                    $active[$type] = true;
                }
            }
        ...
        try {
            $profile = EngineProfile::fromArray(is_array($request['profile'] ?? null) ? $request['profile'] : [], array_keys($active));
        ...
        if ($summary) {
            ...
            foreach ($batch['scenarios'] as $scenario) {
                ...
                foreach (['attacker' => $a, 'defender' => $b] as $side => $camp) {
                    ...
                    $losses = [];
                    $prisoners = 0;
                    foreach ($batch['unitOrder'] as $i => $type) {
                        $cost = $profile->costs()[$type];
                        $initialCost += $r[$side.'InitialByType'][$i] * $cost;
                        $projected = $r[$side.'ProjectedByType'][$i];
                        $lostCost += ($projected[1] + $projected[2]) * $cost;
                        $losses[$type] = ['initial' => $r[$side.'InitialByType'][$i], 'dead' => $projected[2] / 50, 'wounded' => $projected[1] / 50];
                        $prisoners += $projected[3] / 50;
                    }
                    $row['camps'][$camp] = ['losses' => $losses, 'prisoners' => $prisoners, 'winRate' => $r[$side.'Wins'] / 50, 'valueLossRate' => $initialCost > 0 ? $lostCost / 50 / $initialCost : 0];
                }
                $rows[] = $row;
            }
