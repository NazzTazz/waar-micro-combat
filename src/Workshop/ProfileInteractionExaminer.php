...
final readonly class ProfileInteractionExaminer
{
    ...
    private function comparisons(array$results):array{$out=[];foreach($results['P0']['rows']as$row){if($row['side']!=='attacker')continue;$id=$row['scenarioId'];$rates=[];foreach($results as$key=>$measurement)foreach($measurement['rows']as$candidate)if($candidate['scenarioId']===$id&&$candidate['side']==='attacker'){$rates[$key]=$candidate['winRate'];break;}$a=$rates['PA']-$rates['P0'];$b=$rates['PB']-$rates['P0'];$message=$a*$b<0?'Les effets observés vont dans des directions opposées sur cette case.':(($a>0&&$b>0)||($a<0&&$b<0)?'Les effets observés vont dans la même direction sur cette case.':'Au moins un des deux réglages ne change pas le taux de victoire observé sur cette case.');$out[$id]=['winRates'=>$rates,'deltaA'=>$a,'deltaB'=>$b,'message'=>$message];}return$out;}
