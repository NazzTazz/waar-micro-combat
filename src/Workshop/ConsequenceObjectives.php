...
final class ConsequenceObjectives
{
    ...
    public function validate(array $profileValues, array $zones, string $weather, int $seed, int $iterations, ?array $runtime = null): array
    {
        ...
        $expected = [];
        foreach (UnitType::cases() as $a) {
            foreach (UnitType::cases() as $b) {
                foreach (['attacker', 'defender'] as $side) {
                    $expected[$a->value.'-vs-'.$b->value.'/'.$side] = true;
                }
            }
        }
        if (!array_is_list($zones) || count($zones) !== 32) {
            throw new \InvalidArgumentException('Les 32 zones sont requises.');
        }
        $seen = [];
        $evaluator = new AcceptanceZoneEvaluator();
        foreach ($zones as $zone) {
            if (!is_array($zone)) {
                throw new \InvalidArgumentException('Objectif invalide.');
            }
            if (($zone['enabled'] ?? true) !== true) {
                throw new \InvalidArgumentException('Réactivez les 32 objectifs avant la recherche.');
            }
            $id = $zone['id'] ?? null;
            if (!is_string($id) || !isset($expected[$id]) || isset($seen[$id])) {
                throw new \InvalidArgumentException('Objectif inconnu ou dupliqué.');
            }
            $seen[$id] = true;
            $zoneContext = $zone['context'] ?? null;
            $actual = is_array($zoneContext) ? array_diff_key($zoneContext, ['runtime' => true]) : null;
