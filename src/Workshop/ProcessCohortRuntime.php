...
final class ProcessCohortRuntime implements CohortRuntime
{
    ...
    private function call(string $operation, array $request, string $expectedSchema): array
    {
        ...
        try {
            ...
            if (false === fwrite($pipes[0], $payload)) throw new \RuntimeException('Impossible de transmettre le travail au moteur cohortes.');
            fclose($pipes[0]);
        ...
        if (!is_array($result) || array_is_list($result)) throw new \RuntimeException('Réponse objet attendue du moteur cohortes.');
        if (isset($result['error'])) throw new \InvalidArgumentException('Moteur cohortes : '.(string)$result['error']);
    ...
    private function resolvedCommand(): array
    {
        ...
        if ($this->runtimeKind() === 'php') {
            ...
            if (!is_file($script)) throw new \RuntimeException("Runtime PHP cohortes introuvable : {$script}");
    ...
    private function runtimeKind(): string
    {
        ...
        if (!in_array($kind, ['rust', 'php'], true)) throw new \InvalidArgumentException('WAAR_COHORT_RUNTIME doit valoir rust ou php.');
        return $kind;
    }
