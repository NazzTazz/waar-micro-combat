...
final class T27Editor
{
    public function render(array $profile, array $measurement, array $zones): array
    {
        ...
        if (($context['objectiveMetric'] ?? null) !== 'rawCasualtyRatio') {
            throw new \InvalidArgumentException('Mesure obsolète : remesurez les blessés et morts avant compression.');
        }
        (new ConsequenceObjectives())->validate($profile, $zones, $context['weather'] ?? '', $context['baseSeed'] ?? -1, $context['iterations'] ?? 0);
        if (($measurement['profileFingerprint'] ?? null) !== $p->semanticFingerprint() || ($measurement['modelVersion'] ?? null) !== EngineProfile::MODEL_VERSION) {
            throw new \InvalidArgumentException('Mesure incompatible.');
        }
        ...
        foreach ($zones as $z) {
            $editorZones[] = ['id' => $z['id'], 'scenarioId' => $z['scenarioId'], 'side' => $z['side'], 'endpoint' => 'tip', 'xMetric' => 'winRate', 'yMetric' => 'rawCasualtyRatio', 'shape' => 'ellipse', 'center' => $z['center'], 'radii' => $z['radii'], 'enabled' => true, 'approval' => 'draft',
        ...
        $template = str_replace(
            ['<script>__ECHARTS_BUNDLE__</script>', '<script>__ZONES_MODEL__</script>', '<script>__OVERLAY_APP__</script>', '__OVERLAY_JSON__'],
            ['<script src="/editor/echarts.js"></script>', '<script src="/editor/model.js"></script>', '<script src="/editor/app.js"></script>', json_encode($data, JSON_THROW_ON_ERROR | JSON_HEX_TAG | JSON_HEX_AMP)],
            $template
        );
