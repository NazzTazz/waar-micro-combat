...
final class EvolutionaryProfileOptimizer
{
    ...
    {
        if ($budget < 1 || $budget > 256) {
            throw new \InvalidArgumentException('Le budget doit être compris entre 1 et 256 candidats.');
        }
        if ($searchSeed < 0 || $searchSeed > 2147483647) {
            throw new \InvalidArgumentException('Seed de recherche invalide.');
        }
        (new ConsequenceObjectives())->validate($profile, $zones, $weather, $measurementBaseSeed, $iterations);
        $reference = EngineProfile::fromArray($profile)->toArray();
        $zoneMap = [];
        ...
        if (!$dimensions) {
            throw new \InvalidArgumentException('Aucun paramètre ouvert à optimiser.');
        }
        $this->random = new Lcg31($searchSeed);
        $this->dimensionBag = [];
        $this->coverage = array_fill_keys(array_column($dimensions, 'path'), 0);
        ...
        while (count($archive) < $budget) {
            $remaining = $budget - count($archive);
            $size = min(8, $remaining);
            $parents = $this->parents($archive, $dimensions);
            $generationCandidates = [];
            $attempts = 0;
            while (count($generationCandidates) < $size && $attempts < $size * 10) {
                $slot = count($generationCandidates);
                $attempts++;
                if ($generation === 0 && $slot === 0) {
                    $candidateProfile = $reference;
                    $operator = 'reference';
                    $parentIds = [];
                    $mutations = [];
                } else {
                ...
                $parsed = EngineProfile::fromArray($candidateProfile);
                $fingerprint = $parsed->semanticFingerprint();
                if (isset($seen[$fingerprint])) {
                    continue;
                }
                $seen[$fingerprint] = true;
                $candidateProfile = $parsed->toArray();
            ...
            if (!$generationCandidates) {
                $stopReason = 'no_novel_proposals';
                break;
            }
            $before = count($generations) ? $generations[array_key_last($generations)]['best']['metrics'] : null;
            $ranked = $this->rank($archive);
            $best = $ranked[0];
            $metrics = $this->metrics($best);
            $improved = $before === null || $this->compareMetrics($metrics, $before) < 0;
            if ($generation > 0) {
                if ($improved) {
                    $radius = max(.02, $radius * .8);
                    $stagnation = 0;
                } else {
                    $stagnation++;
                    if ($stagnation >= 3) {
                        $radius = min(1, $radius * 2);
                        $stagnation = 0;
                    }
                }
            }
            ...
            $generation++;
            if ($best['inside'] === 32) {
                $stopReason = 'objectives_satisfied';
                break;
            }
        }
        $ranked = $this->rank($archive);
        ...
        foreach ($dimensions as $d) {
            $publicBounds[$d['path']] = array_intersect_key($d, array_flip(['path', 'kind', 'type', 'target', 'field', 'current', 'minimum', 'maximum', 'step']));
        }
    ...
    private function dimensions(array $profile, array $provided): array
    {
        ...
        foreach (self::TYPES as $type) {
            foreach (self::FIELDS as $field) {
                $current = (string)$profile['units'][$type][$field];
                $physicalMax = in_array($field, ['baseAccuracy'], true) ? 1 : (in_array($field, ['defendingEfficiency'], true) ? 10 : 1000);
                $physicalMin = $field === 'structure' ? .000001 : 0;
                $currentUnits = FixedPoint::parse($current);
                $default = $field === 'baseAccuracy' ? ['minimum' => '0', 'maximum' => '1'] : ($field === 'defendingEfficiency' ? ['minimum' => '0', 'maximum' => '10'] : ['minimum' => FixedPoint::format(max((int)round($physicalMin * FixedPoint::SCALE), intdiv($currentUnits, 4))), 'maximum' => FixedPoint::format(min($physicalMax * FixedPoint::SCALE, $currentUnits === 0 ? 10 * FixedPoint::SCALE : $currentUnits * 4))]);
        ...
        $factors = [];
        foreach ($profile['relations'] as $r) {
            $factors[$r['acting']][$r['target']] = $r['factor'];
        }
        foreach (self::TYPES as $acting) {
            foreach (self::TYPES as $target) {
                if ($acting !== $target) {
                    $path = "relations.$acting.$target.factor";
                    $current = (string)($factors[$acting][$target] ?? '1');
                    $dimensions[] = $this->dimension(['path' => $path, 'kind' => 'relation', 'type' => $acting, 'target' => $target, 'current' => $current], $provided[$path] ?? ['minimum' => '0', 'maximum' => '10'], 0, 10);
                }
            }
        }
        if (array_diff_key($provided, array_column($dimensions, null, 'path'))) {
            throw new \InvalidArgumentException('Borne de paramètre inconnue.');
        }
        return $dimensions;
    }
    ...
    private function dimension(array $base, array $raw, float $physicalMin, int $physicalMax): array
    {
        foreach (['minimum', 'maximum'] as $key) {
            if (!is_string($raw[$key] ?? null) && !is_int($raw[$key] ?? null)) {
                throw new \InvalidArgumentException('Borne invalide : '.$base['path']);
            }
        }
        $step = (string)($raw['step'] ?? '0.01');
        $min = FixedPoint::parse($raw['minimum']);
        $max = FixedPoint::parse($raw['maximum']);
        $stepUnits = FixedPoint::parse($step);
        $current = FixedPoint::parse($base['current']);
        if ($stepUnits < 1 || $stepUnits > FixedPoint::SCALE || $min > (int)round($physicalMax * FixedPoint::SCALE) || $min > $max || $max > (int)round($physicalMax * FixedPoint::SCALE) || $min < (int)round($physicalMin * FixedPoint::SCALE) || $current < $min || $current > $max || ($max - $min) % $stepUnits !== 0) {
            throw new \InvalidArgumentException('Bornes ou pas invalides : '.$base['path']);
        }
    ...
    private function parents(array $archive, array $dimensions): array
    {
        ...
        $ranked = $this->rank($archive);
        $parents = array_slice($ranked, 0, min(2, count($ranked)));
        while (count($parents) < min(4, count($ranked))) {
            $chosen = null;
            $distance = -1.0;
            foreach ($ranked as $candidate) {
                if (in_array($candidate['id'], array_column($parents, 'id'), true)) {
                    continue;
                }
                $nearest = INF;
                foreach ($parents as $parent) {
                    $nearest = min($nearest, $this->distance($candidate['profile'], $parent['profile'], $dimensions));
                }
                if ($nearest > $distance) {
                    $distance = $nearest;
                    $chosen = $candidate;
    ...
    {
        $parent = $parents ? $parents[$slot % count($parents)] : ['id' => EngineProfile::fromArray($reference)->semanticFingerprint(), 'profile' => $reference];
        $profile = $parent['profile'];
        $parentIds = [$parent['id']];
        $operator = $slot < 2 ? 'mutation' : ($slot < 4 ? 'multi_mutation' : ($slot < 6 ? 'crossover' : 'global'));
        if ($generation === 0 && $operator === 'crossover') {
            $operator = 'multi_mutation';
        }
        if ($operator === 'crossover' && count($parents) > 1) {
            $other = $parents[($slot + 1) % count($parents)];
            $parentIds[] = $other['id'];
            foreach ($dimensions as $d) {
                if ($this->random->nextIndex(2) === 1) {
                    $this->set($profile, $d, $this->get($other['profile'], $d));
                }
            }
        }
        $count = $operator === 'mutation' ? 1 : 2 + $this->random->nextIndex(3);
        $global = $operator === 'global' || ($stagnation > 0 && $slot >= 4);
        $mutations = [];
        for ($i = 0;$i < min($count, count($dimensions));$i++) {
            $index = $this->nextDimension(count($dimensions));
            $d = $dimensions[$index];
            $before = $this->get($profile, $d);
            $after = $this->mutated($before, $d, $radius, $global);
            if ($after === $before) {
                continue;
            }
            $this->set($profile, $d, $after);
            $this->coverage[$d['path']]++;
            $mutations[] = ['path' => $d['path'], 'before' => $before, 'after' => $after];
        }
    ...
    {
        if (!$this->dimensionBag) {
            $this->dimensionBag = range(0, $count - 1);
            for ($i = $count - 1;$i > 0;$i--) {
                $j = $this->random->nextIndex($i + 1);
                [$this->dimensionBag[$i],$this->dimensionBag[$j]] = [$this->dimensionBag[$j], $this->dimensionBag[$i]];
            }
        }
    ...
    {
        $current = FixedPoint::parse($before);
        $min = $d['min'];
        $max = $d['max'];
        $step = $d['stepUnits'];
        ...
        if ($global) {
            $index = $this->random->nextIndex($count);
        } else {
            $reach = max($step, (int)round(($max - $min) * $radius));
            $low = max($min, $current - $reach);
            $high = min($max, $current + $reach);
            $first = (int)ceil(($low - $min) / $step);
            $last = intdiv($high - $min, $step);
            $localCount = max(1, $last - $first + 1);
            $index = $first + $this->random->nextIndex($localCount);
        }
    ...
    {
        if ($d['kind'] === 'unit') {
            return(string)$profile['units'][$d['type']][$d['field']];
        }
        foreach ($profile['relations'] as $r) {
            if ($r['acting'] === $d['type'] && $r['target'] === $d['target']) {
                return(string)$r['factor'];
            }
        }
        return'1';
    }
    ...
    {
        if ($d['kind'] === 'unit') {
            $profile['units'][$d['type']][$d['field']] = $value;
            return;
        }
        foreach ($profile['relations'] as $i => $r) {
            if ($r['acting'] === $d['type'] && $r['target'] === $d['target']) {
                if ($value === '1') {
                    array_splice($profile['relations'], $i, 1);
                } else {
                    $profile['relations'][$i]['factor'] = $value;
                }
                return;
            }
        }
        if ($value !== '1') {
            $profile['relations'][] = ['acting' => $d['type'], 'target' => $d['target'], 'factor' => $value];
        }
    }
    ...
    private function score(array $measurement, array $zones): array
    {
        $score = 0.0;
        $inside = 0;
        $worst = ['id' => null, 'excess' => -1.0];
        foreach ($measurement['rows'] as $row) {
            $distance = $this->evaluator->normalizedSquaredDistance($row['winRate'], $row['rawCasualtyRatio'], $zones[$row['id']]);
            $excess = $this->penalty->fromSquaredDistance($distance);
            $score += $excess;
            $inside += (int)($excess === 0.0);
            if ($excess > $worst['excess']) {
                $worst = ['id' => $row['id'], 'excess' => $excess];
            }
        }
        return['score' => $score, 'inside' => $inside, 'total' => 32, 'worst' => $worst];
    }

    /** @param list<array<string,mixed>> $candidates @return list<array<string,mixed>> */
    private function rank(array $candidates): array
    {
        usort($candidates, fn ($a, $b) => $this->compareMetrics($this->metrics($a), $this->metrics($b)) ?: ($a['fingerprint'] <=> $b['fingerprint']));
        return$candidates;
    }
    /** @return array{score:float,inside:int,worst:float} */
    private function metrics(array $candidate): array
    {
        return['score' => $candidate['score'], 'inside' => $candidate['inside'], 'worst' => $candidate['worst']['excess']];
    }
    private function compareMetrics(array $a, array $b): int
    {
        return[$a['score'], -$a['inside'], $a['worst']] <=> [$b['score'], -$b['inside'], $b['worst']];
    }
    private function distance(array $a, array $b, array $dimensions): float
    {
        $sum = 0.0;
        foreach ($dimensions as $d) {
            $width = max(1, $d['max'] - $d['min']);
            $sum += abs(FixedPoint::parse($this->get($a, $d)) - FixedPoint::parse($this->get($b, $d))) / $width;
        }
        return$sum / count($dimensions);
    }
}
