...
final class SingleTargetCombatResolver
{
    ...
    public function resolve(PreparedBattle $battle, ?callable $onRound = null): BattleResult
    {
        ...
        for ($round = 1;null === $outcome && $round <= $battle->ruleset->maxRounds;++$round) {
            ...
            $aHealth = $states['attacker']['health'];
            $bHealth = $states['defender']['health'];
            ...
            if (null !== $onRound) {
                $onRound(['number' => $round, 'targetingModel' => self::MODEL, 'before' => $before,
    ...
    {
        if ($army->totalCount() > self::MAX_COMBATANTS_PER_SIDE) {
            throw new \InvalidArgumentException('Le modèle individuel accepte au plus 400 400 combattants par camp.');
        }
        $units = $counts = $structure = $health = $types = [];
        foreach ($army->units() as $unit) {
            $type = $unit->type->value;
            $units[$type] = $unit;
            $counts[$type] = $unit->count;
            $structure[$type] = FixedPoint::checkedMultiply($unit->count, $unit->structureMicro);
            for ($i = 0;$i < $unit->count;++$i) {
                $health[] = $unit->structureMicro;
                $types[] = $type;
            }
        }
    ...
    {
        $damage = array_fill_keys(array_keys($target['units']), 0);
        $trace = [];
        $targetCount = count($target['health']);
        foreach ($acting['units'] as $sourceType => $unit) {
            $sourceCount = $acting['counts'][$sourceType];
            if (0 === $sourceCount) {
                continue;
            }
            ...
            foreach ($impacts as $type => $count) {
                ...
                if (!$tracing || 0 === $target['counts'][$type]) {
                    continue;
                }
    ...
    {
        $counts = $structure = array_fill_keys(array_keys($state['units']), 0);
        $livingHealth = $livingTypes = [];
        foreach ($health as $i => $remaining) {
            if (0 === $remaining) {
                continue;
            }
            $type = $state['types'][$i];
            $livingHealth[] = $remaining;
            $livingTypes[] = $type;
            ++$counts[$type];
            $structure[$type] = FixedPoint::checkedAdd($structure[$type], $remaining);
        }
        $state['health'] = $livingHealth;
        $state['types'] = $livingTypes;
        $state['counts'] = $counts;
    ...
    {
        $snapshot = [];
        foreach ($states as $side => $state) {
            foreach ($state['units'] as $type => $unit) {
                $count = $state['counts'][$type];
                $factor = $side === 'defender' ? $unit->defendingEfficiencyMicro : FixedPoint::SCALE;
    ...
    {
        $a = [] === $states['attacker']['health'];
        $b = [] === $states['defender']['health'];
        return match(true) {
    ...
    private function sideOutcome(array $state): SideOutcome
    {
        $outcomes = [];
        foreach ($state['units'] as $type => $unit) {
            $outcomes[] = new UnitOutcome($unit->type, $state['initial'][$type], $state['counts'][$type], $state['initial'][$type] - $state['counts'][$type], $state['initialStructure'][$type], $state['structure'][$type], $unit->cost);
        }
