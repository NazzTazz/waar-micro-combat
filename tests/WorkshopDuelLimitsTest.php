        ...
        foreach ([0, 8, 100] as $compression) {
            ...
            foreach ($native['directions'] as $i => $direction) {
                ...
                foreach (['attacker', 'defender'] as $side) {
                    $data = $direction['consequences'][$side];
                    $unit = $data['types']['soldier'];
                    ...
                    if ($compression > 0 && $rawWounded > 0) {
                        self::assertGreaterThan(0, $data['economicLoss']);
                    }
                    if ($rawWounded === 0) {
