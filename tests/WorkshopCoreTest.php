...
final class WorkshopCoreTest extends TestCase
{
    ...
    public static function compressionCases(): iterable
    {
        yield 'zero' => [100, 0, 0];
        yield 'eight-of-100' => [100, 8, 8];
        yield 'eight-of-50' => [50, 8, 4];
        yield 'eight-of-7' => [7, 8, 0];
        yield 'full' => [100, 100, 100];
    }
    ...
    {
        $profile = EngineProfile::defaults();
        $profile['combat']['lossCompressionPercent'] = 8;
        $profile['combat']['capturePercent'] = 10;
        $profile['units']['soldier']['capturable'] = true;
        $raw = $this->fixtureResult(1000, CombatSide::Defender, 1000);
        $before = $raw->toArray();
        $result = (new BattleConsequences())->apply($raw, EngineProfile::fromArray($profile));
    ...
    {
        $profile = EngineProfile::defaults();
        $profile['combat']['lossCompressionPercent'] = 0;
        $profile['combat']['capturePercent'] = 10;
        $profile['units']['soldier']['capturable'] = true;
    ...
    public function testNeutralWeatherIsBitExactAndWeatherOnlyChangesPreparedAttack(): void
    {
        ...
        $values['weather']['rain']['soldier']['attack'] = '0.5';
        $values['weather']['rain']['soldier']['baseAccuracy'] = '0.8';
    ...
    {
        $profile = EngineProfile::defaults();
        $profile['units']['archer'] = null;
        $profile['units']['knight'] = null;
        $profile['units']['spearman'] = null;
        $result = (new DuelService())->simulate(['requestId' => 'x', 'profile' => $profile, 'armies' => ['A' => ['soldier' => 100], 'B' => ['soldier' => 120]], 'weather' => 'neutral', 'seed' => 42]);
        self::assertSame(['A' => 8000, 'B' => 9600], $result['costs']);
        self::assertSame('A', $result['directions'][0]['labels']['attacker']);
        self::assertSame('B', $result['directions'][1]['labels']['attacker']);
        self::assertSame('x', $result['requestId']);
    }
    ...
    public function testDuelReportsEveryIdentifiableError(): void
    {
        ...
        $profile['units']['archer'] = null;
        try {
            (new DuelService())->simulate(['profile' => $profile, 'armies' => ['A' => ['archer' => 1], 'B' => []], 'weather' => ['A' => 'unknown', 'B' => 'neutral'], 'seed' => -1]);
            self::fail();
        } catch (ProfileValidationException $e) {
            $codes = array_column($e->errors, 'code');
            self::assertContains('missing_unit', $codes);
            self::assertContains('empty_army', $codes);
            self::assertContains('unknown_weather', $codes);
            self::assertContains('invalid_seed', $codes);
        }
    }
    ...
    {
        $profile = EngineProfile::defaults();
        $profile['relations'] = [['acting' => 'soldier', 'target' => 'soldier', 'factor' => '1'], ['acting' => 'x', 'target' => 'knight', 'factor' => '20'], ['acting' => 'archer', 'target' => 'knight', 'factor' => '2'], ['acting' => 'archer', 'target' => 'knight', 'factor' => '2']];
        $profile['combat']['capturePercent'] = 11;
        $errors = EngineProfile::validate($profile);
        $codes = array_column($errors, 'code');
        self::assertContains('diagonal_relation', $codes);
        self::assertContains('unknown_unit', $codes);
        self::assertContains('duplicate_relation', $codes);
    ...
    public function testFingerprintIsSemanticAndDeterministic(): void
    {
        ...
        $a['relations'] = [['acting' => 'knight', 'target' => 'archer', 'factor' => '1.5'], ['acting' => 'archer', 'target' => 'spearman', 'factor' => '2']];
        $b = $a;
        $b['relations'] = array_reverse($b['relations']);
        $b['id'] = 'saved-proposal';
        $b['label'] = 'Proposition renommée';
        $first = EngineProfile::fromArray($a);
        $renamed = EngineProfile::fromArray($b);
    ...
    {
        $profile = EngineProfile::defaults();
        $profile['units']['soldier']['magic'] = 1;
        $profile['relations'][] = ['acting' => 'soldier', 'target' => 'archer', 'factor' => '1.2', 'inverse' => true];
        $profile['weather']['rain']['dragon'] = ['attack' => '1', 'baseAccuracy' => '1'];
        $profile['weather']['rain']['soldier']['accuracySpread'] = '1';
        $profile['combat']['initiative'] = 2;
        $paths = array_column(EngineProfile::validate($profile), 'path');
        foreach (['units.soldier.magic', 'relations.0.inverse', 'weather.rain.dragon', 'weather.rain.soldier.accuracySpread', 'combat.initiative'] as $path) {
    ...
    {
        $profile = EngineProfile::defaults();
        $profile['relations'] = [['acting' => ['soldier'], 'target' => null, 'factor' => []]];
        $errors = EngineProfile::validate($profile);
    ...
    {
        $side = function (int $losses) use ($initial): SideOutcome {
            $units = [];
            foreach (UnitType::cases() as $type) {
                $n = $type === UnitType::Soldier ? $initial : 0;
                $dead = $type === UnitType::Soldier ? $losses : 0;
                $units[] = new UnitOutcome($type, $n, $n - $dead, $dead, $n * 1000000, ($n - $dead) * 1000000, 1);
            }
