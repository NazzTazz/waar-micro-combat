...
final class WorkshopRegressionTest extends TestCase
{
    ...
    public function testRejectsForeignOrMalformedObjectivesBeforeSearch(): void
    {
        ...
        foreach (['profile','seed','weather','iterations','model','unknown','duplicate','radius','missing'] as $case) {
            ...
            switch ($case) {
                ...
                case 'seed': $bad[0]['context']['baseSeed']=314159; break;
                ...
                case 'missing': unset($bad[0]['context']); break;
            }
            try {(new BoundedProfileSearch())->search($profile,$bad,'neutral',314159,1,1);self::fail($case);}
            catch (\InvalidArgumentException $error) {self::assertNotEmpty($error->getMessage(),$case);}
        }
    ...
    {
        $draft=EngineProfile::defaults();$draft['units']['archer']=null;
