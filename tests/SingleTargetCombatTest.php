...
final class SingleTargetCombatTest extends TestCase
{
    ...
    public function testOneFighterCannotDamageTwoTargetsOrSpillLethalDamage(): void
    {
        ...
        $trace=[];$result=(new SingleTargetCombatResolver())->resolve($battle,static function($r) use (&$trace){$trace[]=$r;});
    ...
    public function testEveryLivingFighterStrikesOnceAndContributionsMatchTotals(): void
    {
        ...
        $resolver=new SingleTargetCombatResolver();$trace=[];
        $result=$resolver->resolve($battle,static function($r) use (&$trace){$trace[]=$r;});
        self::assertSame($result->toArray(),$resolver->resolve($battle)->toArray());
        foreach($trace as $i=>$round){
            if($i>0)self::assertSame($trace[$i-1]['after'],$round['before']);
    ...
    {
        $a=$this->army(['soldier'=>1],1,100);$b=$this->army(['archer'=>1],0,3);
        $trace=[];$result=(new SingleTargetCombatResolver())->resolve($this->battle($a,$b,42,3),static function($r) use (&$trace){$trace[]=$r;});
    ...
    {
        $resolver=new SingleTargetCombatResolver();$collision=false;$split=false;
    ...
    public function testDefenseCounterAndPerHitRoundingAreVisible(): void
    {
        ...
        $trace=[];(new SingleTargetCombatResolver())->resolve($battle,static function($r) use (&$trace){$trace[]=$r;});
        $hit=$trace[0]['attacks']['defender'][0];
        self::assertSame(1,$hit['impactCount']);self::assertSame(7500000,$hit['unitAfterDefenseMicro']);
        self::assertSame(15000000,$hit['damagePerHitMicro']);self::assertSame(15000000,$hit['damageMicro']);
    }

    public function testTargetSamplerIsReproducibleAndNotModuloAlternation(): void
    {
        $a=new Lcg31(42);$b=new Lcg31(42);$counts=[0,0,0];
        for($i=0;$i<12000;++$i){$index=$a->nextIndex(3);self::assertSame($index,$b->nextIndex(3));++$counts[$index];}
        foreach($counts as $count){self::assertGreaterThan(3600,$count);self::assertLessThan(4400,$count);}
    }
    ...
    public function testRoundingHappensPerFighterAndZeroDamageStillUsesOneTarget(): void
    {
        ...
        $trace=[];$result=(new SingleTargetCombatResolver())->resolve($this->battle($this->army(['soldier'=>1],0),$tiny),static function($r) use (&$trace){$trace[]=$r;});
    ...
    public function testEmptyBattleAndAllocationLimitAreExplicit(): void
    {
        ...
        self::assertSame(0,$result->roundsPlayed);self::assertNull($result->winner);
    ...
    {
        $trace=[];
        $result=(new SingleTargetCombatResolver())->resolve($this->battle($this->army(['soldier'=>1],1000),$this->army(['archer'=>1,'knight'=>1],0,1),42,3),static function($r) use (&$trace){$trace[]=$r;});
