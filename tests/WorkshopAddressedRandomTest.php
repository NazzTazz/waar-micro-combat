...
final class WorkshopAddressedRandomTest extends TestCase
{
    ...
    {
        foreach($value as &$item)if(is_array($item))$item=self::canonical($item);unset($item);
    ...
    public function testDarthCycleInBothRolesThroughTheRealServices(): void
    {
        ...
        $rules=$fixture['report']['result']['ruleset'];$profile=EngineProfile::defaults();
        foreach($rules['units'] as $unit){$type=$unit['type'];unset($unit['type']);$profile['units'][$type]=$unit;}
        $profile['relations']=[];
        foreach($rules['engagements'] as $a=>$row)foreach($row as $b=>$value)if($a!==$b&&$value['attackFactor']!=='1')$profile['relations'][]=['acting'=>$a,'target'=>$b,'factor'=>$value['attackFactor']];
        ...
        $native=new DuelService(new ProcessCohortRuntime(null,'rust'));$php=new DuelService(new ProcessCohortRuntime(null,'php'));
        $reports=[];
        foreach(['0.25','0.3','0.25'] as $p){$input['profile']['units']['spearman']['baseAccuracy']=$p;
            $result=$native->simulate($input);$reference=$php->simulate($input);
            self::assertSame(self::canonical($reference['directions']),self::canonical($result['directions']));$reports[]=$result;
        ...
        $summary=$native->simulate($input,true);self::assertSame(self::canonical($php->simulate($input,true)),self::canonical($summary));
        self::assertSame(50,$summary['iterations']);self::assertSame(100,$summary['totalCombats']);
    ...
    public function testRuntimeCannotReturnAnotherProtocolOrSwapArmyIdentities(): void
    {
        ...
        ] as $wrong){try{CohortRequestFactory::assertRandomProvenance($wrong,['attacker'=>'A','defender'=>'B']);self::fail('Wrong random identity accepted');}catch(\RuntimeException $e){self::assertStringContainsString('incompatibles',$e->getMessage());}}
    }
}
