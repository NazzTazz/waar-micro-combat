...
const computeLane=WaarWorkshopModel.createComputeLane();
const computeRoutes=new Set(['duel','duel-summary','measure','examine-interaction','search','optimize']);
async function api(path,body){
  const encoded=body?JSON.stringify(body):undefined;
  const request=async()=>{
    const response=await fetch('/api/'+path,{method:body?'POST':'GET',headers:body?{'Content-Type':'application/json'}:{},body:encoded});
    let json;try{json=await response.json()}catch{throw new Error('Réponse serveur invalide (HTTP '+response.status+'). Le calcul a peut-être été interrompu ; consultez le journal PHP.')}
    if(!response.ok||json.ok===false)throw Object.assign(new Error(json.errors?.[0]?.message||'Erreur'),{errors:json.errors||[],status:response.status,retryAfter:WaarWorkshopModel.retryDelay(response.headers?.get('Retry-After'))});
    return json.data;
  };
  return computeRoutes.has(path)?computeLane.run(request):request();
}
function duelSignature(){return JSON.stringify([profile,armies,duelWeather,armyModifiers,$('#duel-seed')?.value])}
let liveTimer=null,liveBusy=false,liveCompleted='',liveReady=false,liveSequence=0,liveMode='duel',manualCompute=false,bench=null;
let liveObserved='',liveResult=null,liveResultMode='',retryAt=0;
const liveGate=WaarWorkshopModel.createRevisionGate(),measurementCache=new Map();
function measurementKey(snapshot,condition,iterations=50){return WaarWorkshopModel.measurementKey({rules:WaarWorkshopModel.profileKey(snapshot)},condition,iterations,42)}
function cachedMeasurement(snapshot,condition,iterations=50){return measurementCache.get(measurementKey(snapshot,condition,iterations))}
async function measureMonotypes(snapshot,condition,iterations=50){
  const key=measurementKey(snapshot,condition,iterations),cached=measurementCache.get(key);if(cached)return cached;
  const response=await api('measure',{profile:snapshot,weather:condition,seed:42,iterations});
  measurementCache.set(key,response);while(measurementCache.size>8)measurementCache.delete(measurementCache.keys().next().value);
  return response;
}
function liveSignature(){return liveMode==='overview'?'overview:'+measurementKey(profile,activeWeather):'duel:'+duelSignature()}
function invalidateLive(){const signature=liveSignature();if(signature!==liveObserved){liveObserved=signature;liveGate.invalidate();liveCompleted=''}}
function scheduleLiveDuel(delay=500){
  if(!liveReady)return;invalidateLive();clearTimeout(liveTimer);
  if(manualCompute||document.hidden||$('#live-duel').hidden||liveSignature()===liveCompleted)return;
  const wait=Math.max(delay,retryAt-Date.now());
  $('#live-duel-status').textContent=retryAt>Date.now()?'Serveur occupé · nouvel essai après le délai demandé.':liveResult?'Résultat précédent · actualisation en cours…':'Calcul en cours…';
  $('#live-duel').classList.add('is-updating');liveTimer=setTimeout(runLiveDuel,wait);
}
async function runLiveDuel(){
  if(liveBusy||manualCompute||document.hidden||$('#live-duel').hidden)return;
  const signature=liveSignature(),mode=liveMode;if(signature===liveCompleted)return;
  if(retryAt>Date.now()){scheduleLiveDuel();return}
  const token=liveGate.capture(signature),snapshot=structuredClone(profile),condition=activeWeather;
  const isCurrent=()=>liveGate.accept(token,liveSignature());
  liveBusy=true;const id=String(++liveSequence);let retry=false;
  try {
    const validation=await api('validate',{profile:snapshot,mode:'complete'});
    if(!isCurrent()||manualCompute||document.hidden||$('#live-duel').hidden)return;
    if(validation.errors.length)throw new Error('Profil incomplet : corrigez les réglages avant le calcul.');
    const result=mode==='overview'?await measureMonotypes(snapshot,condition):await api('duel-summary',{requestId:id,profile:snapshot,armies,weather:duelWeather,modifiers:armyModifiers,seed:Number($('#duel-seed').value)});
    if(!isCurrent()||document.hidden||$('#live-duel').hidden){if(mode==='overview')measurementCache.delete(measurementKey(snapshot,condition));return;}
    if(mode==='duel'&&result.requestId!==id)return;
    liveResult=result;liveResultMode=mode;
    if(mode==='overview'){renderOverview(result);bench.updated()}else renderLiveDuel(result);
    liveCompleted=signature;$('#live-duel-status').textContent='';$('#live-duel').classList.remove('is-updating');$('#live-retry').hidden=true;
  }catch(e){
    if(e.status===429){retryAt=Date.now()+e.retryAfter;retry=true;}
    if(isCurrent()){$('#live-duel-status').textContent=e.message;$('#live-retry').hidden=false;}
  }finally{liveBusy=false;if(!isCurrent()||retry)scheduleLiveDuel();}
}
async function withManualCompute(task){
  if(manualCompute)throw new Error('Une mesure explicite est déjà en cours.');
  if(retryAt>Date.now())throw new Error('Serveur occupé : attendez le délai indiqué avant de réessayer.');
  manualCompute=true;clearTimeout(liveTimer);liveGate.invalidate();liveCompleted='';
  try{return await task()}catch(e){if(e.status===429)retryAt=Date.now()+e.retryAfter;throw e}
  finally{manualCompute=false;scheduleLiveDuel();}
}

function renderLiveDuel(response){const root=$('#live-duel-results');root.className='live-duel-results';root.replaceChildren();const pct=n=>(n*100).toLocaleString('fr-FR',{maximumFractionDigits:1})+' %';for(const row of response.rows){const card=document.createElement('div');card.className='live-duel-direction';const number=n=>n.toLocaleString('fr-FR',{maximumFractionDigits:1}),labels={soldier:'Soldats',spearman:'Lanciers',archer:'Archers',knight:'Chevaliers'},winner=row.camps.A.winRate===row.camps.B.winRate?null:row.camps.A.winRate>row.camps.B.winRate?'A':'B',campClass=camp=>'live-camp-'+camp.toLowerCase()+(winner===camp?' live-winner':'');card.innerHTML=`<div class="live-summary-line"><strong>${row.attacker} attaque ${row.defender}</strong>${['A','B'].map(camp=>'<span class="'+campClass(camp)+'">Victoires '+camp+' '+pct(row.camps[camp].winRate)+'</span>').join('')}${row.drawRate?'<span>Nuls '+pct(row.drawRate)+'</span>':''}</div><table class="live-loss-table" aria-label="Pertes moyennes après compression sur 50 simulations, ${row.attacker} attaque ${row.defender}"><thead><tr><th rowspan="2" scope="col">Camp</th>${Object.values(labels).map(label=>'<th colspan="2" scope="colgroup">'+label+'</th>').join('')}<th rowspan="2" scope="col" title="Unités du camp capturées par l’adversaire">Prisonniers</th></tr><tr>${Object.keys(labels).map(()=>'<th scope="col">Morts</th><th scope="col">Blessés</th>').join('')}</tr></thead><tbody>${['A','B'].map(camp=>'<tr class="live-initial-row"><th scope="row"><span class="'+campClass(camp)+'">'+camp+'</span><small>Effectifs initiaux</small></th>'+Object.keys(labels).map(type=>'<td colspan="2">'+number(row.camps[camp].losses[type].initial)+'</td>').join('')+'<td>—</td></tr><tr><th scope="row"><span class="'+campClass(camp)+'">'+camp+'</span></th>'+Object.keys(labels).map(type=>{const loss=row.camps[camp].losses[type];return '<td>'+number(loss.dead)+'</td><td>'+number(loss.wounded)+'</td>'}).join('')+'<td>'+number(row.camps[camp].prisoners)+'</td></tr>').join('')}</tbody></table>`;root.append(card)}}

function renderOverview(measurement){
  const root=$('#live-duel-results'),selected=bench.selected();root.className='live-duel-results overview-layout';
  const rows=Object.fromEntries(measurement.rows.map(row=>[row.id,row]));
  const pct=n=>(Number(n)*100).toLocaleString('fr-FR',{maximumFractionDigits:1})+' %';
  const matrix=document.createElement('table');matrix.className='overview-matrix';
  matrix.innerHTML='<thead><tr><th>Attaque ↓ / Défense →</th>'+Object.values(types).map(x=>'<th scope="col">'+x+'</th>').join('')+'</tr></thead><tbody>'+Object.keys(types).map(a=>'<tr><th scope="row">'+types[a]+'</th>'+Object.keys(types).map(d=>{
    const row=rows[a+'-vs-'+d+'/attacker'],heat=Math.round(row.winRate*100);
    return '<td><button type="button" data-overview="'+a+'-vs-'+d+'" aria-pressed="'+(selected===a+'-vs-'+d)+'" style="--heat:'+heat+'%;color:'+(heat>55?'#081422':'#eef3f9')+'" aria-label="'+types[a]+' attaquant '+types[d]+', '+pct(row.winRate)+' de victoires">'+pct(row.winRate)+'</button></td>';
  }).join('')+'</tr>').join('')+'</tbody>';
  const detail=document.createElement('div');detail.className='overview-detail';root.replaceChildren();
  const context=document.createElement('div');context.className='overview-context';context.textContent='Monotypes · budget '+measurement.context.budget.toLocaleString('fr-FR')+' par camp · '+weather[measurement.context.weather]+' · 50 simulations par case · couleur : 0 à 100 % de victoires de l’attaquant';
  root.append(context,matrix,detail);
  const a=rows[selected+'/attacker'],d=rows[selected+'/defender'];
  detail.innerHTML='<h3>'+types[a.attackerType]+' attaquant '+types[a.defenderType]+'</h3>'+[a,d].map(row=>'<div class="overview-side"><strong>'+(row.side==='attacker'?'Attaquant':'Défenseur')+' · '+row.initialCount.toLocaleString('fr-FR')+' unités</strong><span>Victoires '+pct(row.winRate)+' · nuls '+pct(row.drawRate)+'</span><span>Brut : morts '+pct(row.rawLossRatio)+' · blessés '+pct(row.rawWoundedRatio)+'</span><span>Après projection : morts '+pct(row.appliedLossRatio)+' · blessés '+pct(row.woundedRatio)+' · prisonniers '+pct(row.captureRatio)+'</span></div>').join('')+'<p class="muted">Pertes en % de l’effectif initial du camp.</p>';
  matrix.querySelectorAll('[data-overview]').forEach(button=>button.onclick=()=>{
    bench.select(button.dataset.overview);
    root.querySelector('[data-overview="'+button.dataset.overview+'"]').focus();
  });
}
function resetFeedbackBranch(){bench?.reset()}

function measurementState(){return JSON.stringify([profile,activeWeather])}
function dirty(){renderProfileMeta();scheduleLiveDuel();bench?.changed();save();invalidateSearch();if(WaarWorkshopModel.stale(resultSignature,duelSignature()))$('#duel-stale').classList.remove('hidden');if(measurement&&measurementSignature!==measurementState()){staleZones=structuredClone(zones);measurement=null;zones=[];editorFingerprint=null;editorLoad++;$('#t27-editor').srcdoc='';$('#zones-empty').hidden=false;$('#zone-editor').classList.add('hidden');$('#search-results').replaceChildren();$('#measure-progress').textContent='Mesure obsolète. La géométrie des zones est conservée et pourra être réassociée après la prochaine mesure.'}}
...
function setLiveMode(mode){liveGate.invalidate();liveMode=mode==='overview'?'overview':'duel';$('#show-live-duel').setAttribute('aria-pressed',String(liveMode==='duel'));$('#show-overview').setAttribute('aria-pressed',String(liveMode==='overview'));liveCompleted='';scheduleLiveDuel(0)}
function selectView(id){
  ...
  if($('#live-duel').hidden){clearTimeout(liveTimer);liveGate.invalidate();liveCompleted='';}else scheduleLiveDuel();
...
async function init(){let stored=null,d=null;try{stored=localStorage.getItem('waar-workshop-draft-v1');d=stored?JSON.parse(stored):null}catch{$('#notice').textContent='Stockage indisponible : utilisez la sauvegarde JSON.'}try{if(d?.profile){const migrated=await api('migrate-profile',{profile:d.profile});profile=migrated.profile;armies=d.armies||armies;activeWeather=weather[d.activeWeather]?d.activeWeather:'neutral';duelWeather=weather[d.duelWeather]?d.duelWeather:(d.duelWeather?.A===d.duelWeather?.B&&weather[d.duelWeather?.A]?d.duelWeather.A:'neutral');armyModifiers=d.armyModifiers||armyModifiers;staleZones=d.staleZones||[];if(migrated.migration.performed)$('#notice').textContent='Profil local migré vers le moteur cohortes. Les anciens résultats sont obsolètes ; les nouveaux réglages restent à confirmer.'}else profile=(await api('default-profile')).profile}catch{profile=(await api('default-profile')).profile}duelWeather=activeWeather;await restoreProfileBaseline(d);
bench=WaarBench.create({root:$('#profile-feedback'),api,getProfile:()=>profile,getWeather:()=>activeWeather,weatherLabels:weather,measure:measureMonotypes,cached:cachedMeasurement,runManual:withManualCompute,onSelect:()=>{if(liveResult&&liveResultMode==='overview')renderOverview(liveResult)}});
  bench.restore();renderAll();setupExpertTools();setupArmyShortcuts();setupSavedProfiles();
  $('#show-live-duel').onclick=()=>setLiveMode('duel');$('#show-overview').onclick=()=>setLiveMode('overview');$('#live-retry').onclick=()=>{liveCompleted='';scheduleLiveDuel(0)};
  document.addEventListener('pointerdown',event=>{if(event.target?.type==='range'&&event.target.closest?.('main'))bench.gesture(true)});
  for(const eventName of ['pointerup','pointercancel'])document.addEventListener(eventName,()=>bench.gesture(false));
  document.addEventListener('visibilitychange',()=>{liveGate.invalidate();liveCompleted='';clearTimeout(liveTimer);if(!document.hidden)scheduleLiveDuel(0)});

liveReady=true;scheduleLiveDuel();
  ...
  $('#simulate').onclick=async()=>{if(liveBusy||manualCompute){errors([{path:'Calcul',message:'Un calcul est déjà en cours ; réessayez à sa fin.'}]);return}manualCompute=true;const id=String(++currentRequest),configurationSignature=duelSignature();errors([]);$('#simulate').disabled=true;try{const response=await api('duel',{requestId:id,profile,armies,weather:duelWeather,modifiers:armyModifiers,seed:Number($('#duel-seed').value)});if(WaarWorkshopModel.responseIsCurrent(response,id,configurationSignature,duelSignature()))renderDuel(response);else $('#duel-stale').classList.remove('hidden')}catch(e){if(id===String(currentRequest))errors(e.errors)}finally{manualCompute=false;if(id===String(currentRequest))$('#simulate').disabled=false;scheduleLiveDuel()}};
  ...
  $('#search').onclick=async()=>{
    ...
    if(liveBusy||manualCompute){$('#search-results').textContent='Un calcul est déjà en cours ; réessayez à sa fin.';return}manualCompute=true;
    ...
    finally{manualCompute=false;$('#search').disabled=false;scheduleLiveDuel()}
  };
