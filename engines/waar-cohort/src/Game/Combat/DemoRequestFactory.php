...
final class DemoRequestFactory
{
    ...
    {
        $rules = WaarRuleset::create('slice-a-demo')->toArray();
        $rules['maxRounds'] = 4;
        foreach ($rules['units'] as &$unit) {
            if ($unit['type'] === 'knight') {
                $unit['strikesPerAttack'] = 5;
            }
        }
        unset($unit);
    ...
    {
        $combat = self::combat('none');
        $scenarios = [];
        $costs = ['soldier' => 10, 'spearman' => 70, 'archer' => 70, 'knight' => 550];
        $types = array_keys($costs);
        foreach ($types as $aIndex => $a) {
            foreach ($types as $dIndex => $d) {
                $scenarios[] = ['id' => "{$a}-vs-{$d}", 'seedKey' => $aIndex * 4 + $dIndex,
