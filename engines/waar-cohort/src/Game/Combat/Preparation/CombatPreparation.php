...
final readonly class CombatPreparation
{
    ...
    public function prepare(CombatRuleset $ruleset, iterable $input = []): PreparedCombatSide
    {
        ...
        foreach ($input as $value) {
            ...
            if(isset($seen[$key])) throw new \InvalidArgumentException("Duplicate modifier: {$modifier->source}/{$modifier->id}");
            $seen[$key]=true;$modifiers[]=$modifier;
        }
        ...
        $units=[];$serialized=array_map(fn(CombatModifier $m)=>$m->toArray(),$modifiers);
        foreach(UnitType::cases() as $type){
            $base=$ruleset->unit($type);$by=[];$bool=[];
            foreach($modifiers as $modifier)if($modifier->unitType===$type){
                if($modifier->parameter==='capturable')$bool[]=$modifier;else$by[$modifier->parameter][]=$modifier;
            }
            ...
            if($bool){$values=array_unique(array_map(fn(CombatModifier $m)=>(int)$m->value,$bool));if(count($values)>1)throw new \InvalidArgumentException("Conflicting capturable modifiers for {$type->value}.");$capturable=(bool)reset($values);}
            $effects=array_values(array_filter($serialized,fn(array $m)=>$m['unitType']===$type->value));
            $unit=new PreparedUnit($type,$decimal('attack',$base->attack),$decimal('structure',$base->structure),$integer('cost',$base->cost),
                $decimal('baseAccuracy',$base->baseAccuracy),$decimal('accuracySpread',$base->accuracySpread),$integer('strikesPerAttack',$base->strikesPerAttack),
                $decimal('defendingEfficiency',$base->defendingEfficiency),$capturable,$base->toArray(),$effects);
    ...
    private function decimal(float $base,array $modifiers):float{return CombatFixedPoint::multiplyMany([$base,...array_map(fn(CombatModifier $m)=>$m->value,$modifiers)]);}
    /** @param list<CombatModifier> $modifiers */
    private function integer(int $base,array $modifiers,string $parameter):int{
        ...
        if($units%CombatFixedPoint::SCALE!==0)throw new \InvalidArgumentException("Prepared {$parameter} must remain an integer.");
