...
final readonly class CombatRuleset
{
    ...
    public function __construct(
        ...
        if ($this->surrenderDeadRatio < 0 || $this->surrenderDeadRatio > 1) {
            throw new \InvalidArgumentException('Surrender threshold must be between 0 and 1.');
        }
        if ($this->woundDamageThreshold < 0 || $this->woundDamageThreshold > 1) {
            throw new \InvalidArgumentException('Wound damage threshold must be between 0 and 1.');
        }
        if (!in_array($tieBreakCriterion, ['economic', 'structure'], true)) {
            throw new \InvalidArgumentException('Unknown tie-break criterion.');
        }
        if (!in_array($equalityPolicy, ['defender', 'draw'], true)) {
            throw new \InvalidArgumentException('Unknown equality policy.');
        }
        $indexed = [];
        foreach ($definitions as $definition) {
            if (isset($indexed[$definition->type->value])) {
                throw new \InvalidArgumentException('Duplicate unit definition.');
            }
            $indexed[$definition->type->value] = $definition;
        }
        if (count($indexed) !== count(UnitType::cases())) {
            throw new \InvalidArgumentException('A definition is required for every unit type.');
        }
        $this->definitions = $indexed;
    }

    public function unit(UnitType $type): UnitDefinition
    {
    ...
    {
        return array_map(fn (UnitType $type) => $this->unit($type), UnitType::cases());
    }
    public function targetWeight(UnitType $acting, UnitType $target): float
    {
        return $this->targeting->weight($acting, $target);
    }
    public function attackFactor(UnitType $acting, UnitType $target): float
    {
        return $this->engagements->get($acting, $target)->attackFactor;
    }
    ...
    public function toArray(): array
    {
        ...
        if (null !== $this->serializedWoundDamageThreshold) {
            $data['woundDamageThreshold'] = CombatFixedPoint::format($this->woundDamageThreshold);
        }
        return $data;
    }
    ...
    {
        if ($unknown = array_diff(array_keys($data), ['schemaVersion', 'modelVersion', 'version', 'units', 'targetingMode', 'targeting', 'engagements', 'maxRounds', 'surrender', 'tieBreak', 'woundDamageThreshold'])) {
            throw new \InvalidArgumentException('Unknown ruleset field: '.reset($unknown));
        }
        if (($data['schemaVersion'] ?? self::SCHEMA_VERSION) !== self::SCHEMA_VERSION || ($data['modelVersion'] ?? self::MODEL_VERSION) !== self::MODEL_VERSION) {
            throw new \InvalidArgumentException('Unsupported cohort ruleset schema or model.');
        }
        if (($data['targetingMode'] ?? 'proportional') !== 'proportional') {
            throw new \InvalidArgumentException('waar-cohort-v2 only supports proportional targeting.');
        }
        $surrender = (array) ($data['surrender'] ?? []);
        $tieBreak = (array) ($data['tieBreak'] ?? []);
        if (array_diff(array_keys($surrender), ['enabled', 'deadRatio']) || array_diff(array_keys($tieBreak), ['criterion', 'equality'])) {
            throw new \InvalidArgumentException('Unknown surrender or tie-break field.');
        }
        return new self(
            (string) ($data['version'] ?? ''),
            ...
            self::integer($data, 'maxRounds', 3),
            self::boolean($surrender, 'enabled', false),
            $surrender['deadRatio'] ?? '0.2',
            (string) ($tieBreak['criterion'] ?? 'economic'),
            (string) ($tieBreak['equality'] ?? 'defender'),
