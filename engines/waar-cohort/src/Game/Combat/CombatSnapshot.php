...
final readonly class CombatSnapshot
{
    ...
    public function __construct(
        ...
        public int $seed,
        public PreparedCombatSide $attacker,
        public PreparedCombatSide $defender,
        public string $traceLevel = 'full',
        public StochasticEngineVersion $stochasticEngineVersion = StochasticEngineVersion::Lcg31NormalApproximationV1,
        public string $numericModelVersion = CombatFixedPoint::VERSION,
        public ?array $armyIdentities = null
    ) {
        if (!in_array($traceLevel, ['none', 'full'], true)) {
            throw new \InvalidArgumentException('Trace level must be none or full.');
        }
        if ($stochasticEngineVersion === StochasticEngineVersion::AddressedBinomialV1) {
            if ($armyIdentities === null || count($armyIdentities) !== 2
                ...
                || $armyIdentities['attacker'] === $armyIdentities['defender']) {
                throw new \InvalidArgumentException('Distinct A/B armyIdentities are required.');
            }
        } elseif ($armyIdentities !== null) {
            throw new \InvalidArgumentException('Legacy protocol cannot use armyIdentities.');
        }
    }
    ...
    {
        return ['schemaVersion' => self::SCHEMA_VERSION, 'rulesetVersion' => $this->rulesetVersion, 'seed' => $this->seed, 'traceLevel' => $this->traceLevel,
