...
{
    private RandomGeneratorFactory $randomFactory;private AccuracySampler $accuracySampler;
    ...
    {$this->randomFactory=$randomFactory??new RandomGeneratorFactory();$this->accuracySampler=$accuracySampler??new AccuracySampler();}
    ...
    {
        if($snapshot->rulesetVersion!==$ruleset->version||$snapshot->numericModelVersion!==CombatFixedPoint::VERSION)throw new \InvalidArgumentException('Snapshot and ruleset versions do not match.');
        if($attacker->initialCount()===0&&$defender->initialCount()===0)throw new \InvalidArgumentException('Both armies cannot be empty.');
        ...
        for($round=1;$round<=$ruleset->maxRounds;++$round){$attackerStart=clone $attacker;$defenderStart=clone $defender;
            ...
            if($aEmpty||$dEmpty){if($aEmpty&&$dEmpty){[$winner,$decision]=$this->tieBreak($attacker,$defender,$snapshot->attacker,$snapshot->defender,$ruleset,'double_elimination');}
                else{$winner=$aEmpty?CombatSide::Defender:CombatSide::Attacker;$decision=['criterion'=>'elimination','attackerLiving'=>$attacker->livingCount(),'defenderLiving'=>$defender->livingCount()];}
                return $this->result($attacker,$defender,$rounds,$winner,VictoryReason::Elimination,$snapshot,$ruleset,$decision,$hash);}
            if($ruleset->surrenderEnabled){$aSurrenders=$attacker->reachesDeathRatio($ruleset->surrenderDeadRatio);$dSurrenders=$defender->reachesDeathRatio($ruleset->surrenderDeadRatio);
                if($aSurrenders||$dSurrenders){if($aSurrenders&&$dSurrenders)[$winner,$decision]=$this->tieBreak($attacker,$defender,$snapshot->attacker,$snapshot->defender,$ruleset,'double_surrender');
                    else{$winner=$aSurrenders?CombatSide::Defender:CombatSide::Attacker;$decision=['criterion'=>'surrender','threshold'=>CombatFixedPoint::format($ruleset->surrenderDeadRatio),'attackerDeadRatio'=>$attacker->deathRatioText(),'defenderDeadRatio'=>$defender->deathRatioText()];}
    ...
    {
        if($ruleset->tieBreakCriterion==='economic'){$a=$attacker->remainingValue($aPrepared);$d=$defender->remainingValue($dPrepared);$comparison=$a<=>$d;$values=['attacker'=>$a,'defender'=>$d];}
        else{$an=$attacker->totalStructureUnits();$ad=$attacker->initialStructureUnits($aPrepared);$dn=$defender->totalStructureUnits();$dd=$defender->initialStructureUnits($dPrepared);
            $comparison=CombatFixedPoint::compareProducts($an,$dd,$dn,$ad);$values=['attacker'=>['remainingUnits'=>$an,'initialUnits'=>$ad],'defender'=>['remainingUnits'=>$dn,'initialUnits'=>$dd]];}
        if($comparison===0)$winner=$ruleset->equalityPolicy==='defender'?CombatSide::Defender:null;else$winner=$comparison>0?CombatSide::Attacker:CombatSide::Defender;
