...
final class RoundResolver
{
    ...
    {
        $matrix=[];foreach(UnitType::cases() as $acting)foreach(UnitType::cases() as $target){$unit=$prepared->unit($acting);$perStrike=CombatFixedPoint::divideByInt($unit->attack,$unit->strikesPerAttack);
            $damage=CombatFixedPoint::multiply($perStrike,$ruleset->attackFactor($acting,$target));if($defending)$damage=CombatFixedPoint::multiply($damage,$unit->defendingEfficiency);
    ...
    {
        if(CombatFixedPoint::compare($damage,0)<=0)return null;$needed=0;
        foreach($army->cohorts($type) as $cohort)$needed+=CombatFixedPoint::ceilRatio($cohort->remainingStructure,$damage)*$cohort->count;
        return $needed;
    }
    ...
    private function applyImpacts(CombatArmy $army,UnitType $type,int $impacts,float $damage,RandomSource $random,?AddressedRandom $addressed=null,string $acting='',int $wave=0):int
    {
        ...
        $updated=[];$deaths=0;foreach($cohorts as $index=>$cohort){$perUnit=intdiv($allocations[$index],$cohort->count);$remainder=$allocations[$index]%$cohort->count;
            ...
            if($remainder>0)$this->appendDamaged($updated,$deaths,$cohort,$remainder,$perUnit+1,$damage);
        }$army->replaceType($type,$updated,$deaths);$after=0;foreach($army->cohorts($type) as $cohort)$after+=CombatFixedPoint::units($cohort->remainingStructure)*$cohort->count;
        return $before-$after;
    }
    ...
    {
        $structure=CombatFixedPoint::subtractRepeated($source->remainingStructure,$hits,$damage);if(CombatFixedPoint::compare($structure,0)<=0)$deaths+=$count;
        else $updated[]=new UnitCohort($source->type,$structure,$count);
    }
}
