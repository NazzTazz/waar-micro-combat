...
final class ConsequencePolicy
{
    ...
    {
        if (!in_array($version, [self::VERSION, self::PROBABILISTIC_VERSION, self::ADDRESSED_VERSION], true)) throw new \InvalidArgumentException('Unsupported consequence policy.');
    }

    public static function samplingProtocol(string $version): string
    {self::validateVersion($version);return match($version){self::VERSION=>'floor/1',self::PROBABILISTIC_VERSION=>ConsequenceSampler::VERSION,self::ADDRESSED_VERSION=>AddressedRandom::VERSION};}
    ...
    public function project(CombatResult $result,CombatRuleset $ruleset,int $compressionPercent,int $capturePercent,string $policyVersion=self::VERSION):array
    {
        ...
        if (($policyVersion===self::ADDRESSED_VERSION)!==($result->snapshot->armyIdentities!==null))throw new \InvalidArgumentException('Consequence and stochastic protocols are incompatible.');
