...
final class AccuracySampler
{
    ...
    {
        if (!in_array($role, ['attacker', 'defender'], true) || $round < 1) {
            throw new \InvalidArgumentException('Invalid accuracy substream identity.');
        }
        ...
        if ($lower === $upper) {
            ...
            $range = $upper - $lower + 1;
            $bucket = intdiv(self::MODULUS, $range);
            $limit = $bucket * $range;
            do {
                $state = (int)((1103515245 * $state + 12345) % self::MODULUS);
            } while ($state >= $limit);
