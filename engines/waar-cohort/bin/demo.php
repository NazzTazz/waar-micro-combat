...
$php=new CombatEngine();$rust=new RustCombatResolver(dirname(__DIR__));$request=DemoRequestFactory::combat();
$started=hrtime(true);$phpResult=$php->resolveRequest($request);$phpMs=(hrtime(true)-$started)/1e6;
$started=hrtime(true);$rustResult=$rust->resolveRequest($request);$rustMs=(hrtime(true)-$started)/1e6;
$started=hrtime(true);$batch=$rust->resolveBatch(DemoRequestFactory::batch());$batchMs=(hrtime(true)-$started)/1e6;
function firstDifference(mixed $left,mixed $right,string $path='$'):?array{if(gettype($left)!==gettype($right)||(!is_array($left)&&$left!==$right))return ['path'=>$path,'php'=>$left,'rust'=>$right];if(is_array($left)){foreach(array_unique([...array_keys($left),...array_keys($right)])as$key){if(!array_key_exists($key,$left)||!array_key_exists($key,$right))return ['path'=>$path.'.'.$key,'php'=>$left[$key]??null,'rust'=>$right[$key]??null];$difference=firstDifference($left[$key],$right[$key],$path.'.'.$key);if(null!==$difference)return$difference;}}return null;}
