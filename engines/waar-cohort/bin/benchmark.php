...
$duel = DemoRequestFactory::combat('none');
$started = hrtime(true);
for ($i = 0;$i < 100;++$i) {
    $duel['seed'] = $i;
    $last = $rust->resolveRequest($duel);
}
$duelSeconds = (hrtime(true) - $started) / 1e9;
$raw = DemoRequestFactory::batch();
unset($raw['consequences']);
$started = hrtime(true);
$rawResult = $rust->resolveBatch($raw);
$rawSeconds = (hrtime(true) - $started) / 1e9;
$projected = DemoRequestFactory::batch();
$started = hrtime(true);
$projectedResult = $rust->resolveBatch($projected);
$projectedSeconds = (hrtime(true) - $started) / 1e9;
