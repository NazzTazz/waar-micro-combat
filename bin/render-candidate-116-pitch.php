...
try {
    ...
    if (count($matches) !== 1) throw new RuntimeException('Candidate 116 missing or duplicated.');
    $candidate = $matches[0];
    ...
    foreach (['plan.json','legacy-reference.json'] as $file) {
        if (($manifest[$file] ?? null) !== hash_file('sha256',$legacyDirectory.'/'.$file)) throw new RuntimeException('Legacy artifact hash mismatch.');
    }
    $legacy = $read($legacyDirectory.'/legacy-reference.json');
    $plan = $read($legacyDirectory.'/plan.json');
    if ($plan['experimentSha256'] !== hash_file('sha256',$run.'/experiment.json')) throw new RuntimeException('Legacy corpus plan mismatch.');
    ...
    $legacyRows = [];
    foreach ($legacy['rows'] as $row) $legacyRows[$row['scenarioId'].'/'.$row['side']] = $row;
    ...
    foreach ($comparison['scenarios'] as $scenario) {
        ...
        foreach ($rows as $row) {
            ...
            if ($point['legacy'][1] > 110) throw new RuntimeException('Legacy point outside fixed display domain.');
    ...
    if (!is_dir($output) && !mkdir($output,0777,true)) throw new RuntimeException('Cannot create output.');
    foreach (['report.html'=>$html, 'presentation.json'=>$encode($data)] as $name=>$content) {
        if (file_put_contents($output.'/'.$name,$content) === false) throw new RuntimeException('Cannot write '.$name);
    }
