...
try {
    if ($argc > 2 || file_exists($output)) {
        throw new RuntimeException('Output directory must be absent.');
    }
    $host = realpath($root.'/../waar-v3');
    if (!$host) {
        throw new RuntimeException('Sibling Legacy oracle unavailable.');
    }
    ...
    foreach ($reference['reference']['sourceFiles'] as $source) {
        if (hash_file('sha256', $host.'/'.$source['path']) !== $source['sha256']) {
            throw new RuntimeException('Legacy source changed: '.$source['path']);
        }
    }
    ...
    if ($preflight['rows'] !== $reference['rows']) {
        throw new RuntimeException('Legacy T24 reproduction failed.');
    }
    $experimentPath = $root.'/experiments/references/t31-standard-seed-314159/experiment.json';
    $experiment = $read($experimentPath);
    if (count($experiment['scenarios']) !== 16 || $experiment['iterations'] !== 200 || $experiment['baseSeed'] !== 42) {
        throw new RuntimeException('Unexpected sampling.');
    }
    ...
    foreach ($reference['reference']['sourceFiles'] as $source) {
        if (hash_file('sha256', $host.'/'.$source['path']) !== $source['sha256']) {
            throw new RuntimeException('Legacy source changed during observation.');
        }
    }
