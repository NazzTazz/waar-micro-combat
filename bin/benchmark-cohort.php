...
if ($timed && (isset($options['combats']) || isset($options['seconds']))) {
    fwrite(STDERR, "Use --duration on its own, without --combats or --seconds.\n");
    exit(2);
}
...
if (isset($options['output']) && file_exists($options['output'])) {
    fwrite(STDERR, "Output already exists; choose another path.\n");
    exit(2);
}
...
$army = static function (array $shares) use ($profile): array {
    $army = [];
    foreach (array_keys($profile->units) as $i => $type) {
        $army[$type] = intdiv(400400 * $shares[$i], 100 * $profile->costs()[$type]);
    }
    return $army;
};
foreach ($shares as $a => $left) {
    foreach ($shares as $d => $right) {
        $mixed['scenarios'][] = ['id' => "$a-vs-$d", 'armyIdentities' => ['attacker' => 'A', 'defender' => 'B'], 'attacker' => ['units' => $army($left), 'modifiers' => $profile->weatherModifiers('neutral', 'attacker')], 'defender' => ['units' => $army($right), 'modifiers' => $profile->weatherModifiers('neutral', 'defender')]];
    }
}
$combined = $base;
$combined['scenarios'] = [...$base['scenarios'], ...$mixed['scenarios']];
...
$report = ['schemaVersion' => 'waar-cohort-paired-benchmark/1', 'timestamp' => gmdate(DATE_ATOM), 'machine' => gethostname(), 'os' => php_uname(), 'php' => PHP_VERSION,
    ...
    'workloads' => []];
if ($timed) {
    $report['method'] = 'Fixed-duration wall-clock throughput window per engine, including process startup, JSON, combat, consequences and driver bookkeeping; no HTTP. Each batch contains all 16 monotypes and 16 mixed scenarios equally. One warm-up batch excluded. Complete final batch before stopping; actual elapsed time reported.';
}
foreach ($workloads as $name => $template) {
    $scenarioCount = count($template['scenarios']);
    $targetIterations = $timed ? PHP_INT_MAX : intdiv($count, 32);
    $phpHashes = [];
    $phpTimes = [];
    $measurements = [];
    foreach (['php', 'rust'] as $kind) {
        $runtime = new ProcessCohortRuntime(null, $kind);
        $warm = $runtime->batch($template);
        $warmHash = $fingerprint($warm);
        if ($kind === 'php') {
            $referenceWarm = $warmHash;
        } elseif ($warmHash !== $referenceWarm) {
            throw new RuntimeException("Warm-up parity failed: $name");
        }
        $done = 0;
        $elapsed = 0.0;
        $matched = 0;
        ...
        while ($done < $targetIterations && $elapsed < $seconds) {
            $request = $template;
            $request['iterations'] = $request['totalIterations'] = min($batch, $targetIterations - $done);
            ...
            if ($kind === 'php') {
                $phpHashes[$lot] = $hash;
                $phpTimes[$lot] = $duration;
            } elseif (isset($phpHashes[$lot])) {
                if ($hash !== $phpHashes[$lot]) {
                    throw new RuntimeException("Result parity failed: $name batch $lot");
                }
                $matched += $result['totalCombats'];
                $matchedSeconds += $duration;
            }
            $elapsed += $duration;
            $done += $request['iterations'];
            $timings[] = $duration;
            $lot++;
            if ($timed) {
                $elapsed = (hrtime(true) - $windowStarted) / 1e9;
            }
        }
...
if (isset($options['output'])) {
    if (file_exists($options['output'])) {
        throw new RuntimeException('Output already exists; choose another path.');
    }
    if (file_put_contents($options['output'], $json, LOCK_EX) === false) {
