...
$options=getopt('', ['combats:','seconds:','batch:','output:','profile:']);
$count=filter_var($options['combats']??100000,FILTER_VALIDATE_INT);
$seconds=filter_var($options['seconds']??30,FILTER_VALIDATE_FLOAT);
$batch=filter_var($options['batch']??5,FILTER_VALIDATE_INT);
...
$report=['schemaVersion'=>'waar-cohort-paired-benchmark/1','timestamp'=>gmdate(DATE_ATOM),'machine'=>gethostname(),'os'=>php_uname(),'php'=>PHP_VERSION,
    'profileId'=>$profile->id,'profileFingerprint'=>$profile->semanticFingerprint(),'requestedCombatsPerEngine'=>$count,'secondsPerEngineWorkload'=>$seconds,'batchIterations'=>$batch,
    ...
    'workloads'=>[]];
foreach(['monotypes'=>$base,'mixed'=>$mixed]as$name=>$template){
    $targetIterations=intdiv($count,32);$phpHashes=[];$phpTimes=[];$measurements=[];
    foreach(['php','rust']as$kind){
        ...
        $combats=$done*16;
        $measurements[$kind]=['combats'=>$combats,'seconds'=>$elapsed,'combatsPerSecond'=>$combats/$elapsed,'completed'=>$done===$targetIterations,'batches'=>$lot,'batchSecondsMin'=>min($timings),'batchSecondsMax'=>max($timings),'estimatedSecondsForWorkloadTarget'=>($count/2)*$elapsed/$combats];
        if($kind==='rust'){
        ...
        fwrite(STDERR,sprintf("%s %s: %d / %d combats, %.2f s, %.1f combats/s%s\n",$name,$kind,$combats,$count/2,$elapsed,$combats/$elapsed,$done===$targetIterations?'':' (time budget reached)'));
    }
