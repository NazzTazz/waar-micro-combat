(() => {
  ...
  function validate(data) {
    if (!data || data.schemaVersion !== 'waar-monotype-finalist-stability-presentation/0.1') throw new Error('Schéma de stabilité T33 incompatible.');
    ...
    if (!Array.isArray(data.scenarios) || data.scenarios.length !== 16) throw new Error('Le rapport T33 doit contenir 16 confrontations.');
    if (data.browserContract?.simulationAllowed !== false || data.browserContract?.objectiveInferenceAllowed !== false) throw new Error('Le contrat de lecture seule T33 est absent.');
