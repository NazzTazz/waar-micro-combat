(() => {
  'use strict';
  const diagnostics = globalThis.__waarT32Diagnostics = {errors: [], viewChanges: 0, simulations: 0, exports: [], objectiveCount: 0, echartsVersion: null};
  try {
    ...
    if (!model) throw new Error('Modèle de présentation T32 absent.');
    model.validate(data);
    ...
    function renderTable(view) {
      ...
      $('#provenance').innerHTML = `Plan <span class="mono">${escapeHtml(data.run.planId)}</span> · candidat <span class="mono">${escapeHtml(view.candidate.id)}</span> · variante <span class="mono">${escapeHtml(view.candidate.source.variant.sha256)}</span> · évaluation <span class="mono">${escapeHtml(view.candidate.source.evaluation.sha256)}</span> · objectifs <span class="mono">${escapeHtml(data.objectiveDocument.sourceSha256)}</span>`;
    }
    ...
    const node = document.querySelector('#chart-error'); if(node){node.style.display='block';node.textContent=`Rapport T32 invalide : ${error.message}`;}
  }
})();
